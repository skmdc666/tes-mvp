...
function requestTiming(req, res, next) {
  ...
  res.on('finish', () => {
    ...
    // Add timing header for client if not already set
    if (!res.headersSent) {
...
function errorTracking(err, req, res, next) {
  if (err) {
    ...
    // Add error correlation ID if not already set
    if (!res.headersSent) {
...
export function createMonitoring(app) {
...
export { rateLimiter, speedLimiter, requestTiming, errorTracking, metricsEndpoint };